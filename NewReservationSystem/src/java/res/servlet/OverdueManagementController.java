...
@WebServlet(name = "OverdueManagementController",urlPatterns = {"/management/overdueManagement"})
public class OverdueManagementController extends HttpServlet {
    EquipDB equipDB;
    OverdueDB overdueDB;
    public void init(){
    ...
    public void doFetchData(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        ArrayList<RecordBean> rb = overdueDB.fetchOverdueRecord(0);
        request.setAttribute("overdueList", rb);
        RequestDispatcher rd = getServletContext().getRequestDispatcher("/pages/management/overdue_management.jsp");
    ...
    public void doSearch(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        UserBean targetUserBean = (UserBean) request.getSession().getAttribute("userInfo");
        ArrayList<RecordBean> rb = overdueDB.searchRecord(0,equipment);
        request.setAttribute("overdueList", rb);
        RequestDispatcher rd = getServletContext().getRequestDispatcher("/pages/management/overdue_management.jsp");
        rd.forward(request, response);
    }
